...
public class RoadmapEntryController {
        ...
        public ResponseEntity<RoadmapEntry> createRoadmapEntry(@RequestBody RoadmapEntry roadmapEntry) {
                ...
                System.out.println(
                                "DEBUG (Controller - POST): Received data for new entry - Category: "
                                                + roadmapEntry.getCategoryName() +
                ...
                if (existingCategory.isEmpty()) {
                        ...
                        System.out.println("DEBUG (Controller - POST): RoadmapCategory already exists for Name='"
                                        + combinedName
                        ...
                        System.out.println(
                                        "DEBUG (Controller - POST): Existing RoadmapCategory updated (updated_at): Name='"
                ...
                System.out.println("DEBUG (Controller - POST): RoadmapEntry saved with ID: " + savedEntry.getId() +
                                ", StartMonth: " + savedEntry.getStartMonth() + ", EndMonth: "
                                + savedEntry.getEndMonth() +
        ...
        public List<RoadmapEntry> getAllRoadmapEntries() {
                ...
                System.out.println(
                                "DEBUG (Controller - GET): Found " + entries.size() + " roadmap entries for user ID: "
                                                + currentUserId);
                return entries;
        }
        ...
        public ResponseEntity<RoadmapEntry> updateRoadmapEntry(@PathVariable Long id,
                ...
                if (currentUserOptional.isEmpty()) {
                        System.out.println(
                                        "DEBUG (Controller - PUT): AppUser not found for username: " + username
                                                        + ". Returning FORBIDDEN.");
                        return ResponseEntity.status(HttpStatus.FORBIDDEN).build();
                }
                AppUser currentUser = currentUserOptional.get(); // ユーザー情報を取得
                ...
                if (existingEntryOptional.isEmpty()) {
                        System.out.println(
                                        "DEBUG (Controller - PUT): RoadmapEntry with ID " + id
                                                        + " not found. Returning NOT_FOUND.");
                ...
                if (existingCategory.isEmpty()) {
                        ...
                        roadmapCategoryRepository.save(existing);
                        System.out.println(
                                        "DEBUG (Controller - PUT): Existing RoadmapCategory updated (updated_at): Name='"
                                                        + combinedName + "' for User ID=" + currentUserId);
                }
        ...
        public ResponseEntity<HttpStatus> deleteRoadmapEntry(@PathVariable Long id) {
                ...
                if (entryToDeleteOptional.isEmpty()) {
                        System.out.println(
                                        "DEBUG (Controller - DELETE): RoadmapEntry with ID " + id
                                                        + " not found. Returning NOT_FOUND.");
                        return ResponseEntity.notFound().build();
                }
